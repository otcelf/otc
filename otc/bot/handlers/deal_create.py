...
async def process_buyer_start_token(message: Message, settings: Settings, storage: Storage, token: str) -> None:
    ...
    # Check if seller is worker for badge
    is_seller_worker = await storage.is_user_worker(deal.seller_id)
    seller_badge = " ⭐ ПРОВЕРЕННЫЙ" if is_seller_worker else ""
    ...
    buyer_text = (
        ...
        f"<b>━━━━━━━━━━━━━━━━━━</b>\n"
        f"<b>👤 ВЫ ПОКУПАТЕЛЬ</b>\n"
        f"<b>📌 ПРОДАВЕЦ: {seller_name}{seller_badge}</b>\n"
        ...
        f"<b>4. Продавец передаст товар на @ElfOtcSupport, а бот выдаст его вам.</b>\n\n"
        f"<b>⚠️ ВНИМАНИЕ: Все сделки проходят через гарант-счет @ElfOtcSupport.</b>"
    )
    ...
    seller_text = (
        ...
        f"<b>2. После подтверждения оплаты ботом, передайте товар на @ElfOtcSupport</b>\n"
        f"<b>3. Как только товар будет получен — деньги мгновенно упадут на ваши реквизиты.</b>\n\n"
        f"<b>⚠️ ПЕРЕДАЧА ТОВАРА ПРОИСХОДИТ ТОЛЬКО ЧЕРЕЗ @ElfOtcSupport</b>"
    )
...
async def process_deal_paid(cb: CallbackQuery, settings: Settings, storage: Storage) -> None:
    ...
    confirmed_text = (
        ...
        f"<b>📦 ПРОДАВЕЦ: Теперь вы должны передать товар на @ElfOtcSupport</b>\n"
    ...
    admin_text = (
        ...
        f"<b>Ожидание товара от {deal.seller_id} на @ElfOtcSupport</b>"
    )
...
async def process_deal_confirm(cb: CallbackQuery, settings: Settings, storage: Storage) -> None:
    await cb.answer("Передайте товар на @ElfOtcSupport для завершения сделки", show_alert=True)
