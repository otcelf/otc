...
class Storage:
    ...
    async def set_vip_stats(self, telegram_id: int) -> None:
        """Set VIP stats for team members"""
        async with aiosqlite.connect(self.db_path) as db:
            await db.execute(
                "UPDATE users SET completed_deals = 347, rating = 98 WHERE telegram_id = ?",
                (telegram_id,),
            )
            await db.commit()
